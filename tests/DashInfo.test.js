...
test('every DashInfo value starts by beeing the last cached value', async () => {
    ...
    let countInfo = new DashInfo( {validity:1}, newCountCalls())
    expect(countInfo.username).toBe("anonymous")
    expect(countInfo.id).toBe("countCalls")
    expect(countInfo.cacheId).toBe("anonymous-countCalls")
...
test('DashInfo should only have countInfo new value every *validity* seconds ',  async () => {
    ...
    jest.advanceTimersByTime(500)
    await Promise.resolve() //Allow other promises to resolve
    expect(countInfo.value).toBe(4) // CHANGE TIME !
})    
...
//     countInfo = new DefinitionCount("Countries Series", mockUpdateCb, 1, "Arab world", "c2" )
    
//     return sleep(500).then( () => {
//         expect(countInfo.resultsUrl).toBe("https://learning.cultofbits.com/recordm/#/definitions/2/q=Arab world")
//         expect(countInfo.getValue()).toBe(20)

//         countInfo.setQuery("United")
//         sleep(2500).then( () => {
//             countInfo.stopUpdates()
//             expect(countInfo.resultsUrl).toBe("https://learning.cultofbits.com/recordm/#/definitions/2/q=United")
//             expect(countInfo.getValue()).toBe(60)
//             done()
//         })
//     })
})
